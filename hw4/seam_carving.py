...
# print(np.gradient(np.array([[1, 2, 6], [3, 4, 5]], dtype=float), axis=1))
...
def energy_function(image):
    ...
    ### YOUR CODE HERE
    Ex = np.gradient(gray_image, axis=1)
    Ey = np.gradient(gray_image, axis=0)

    out = np.abs(Ex) + np.abs(Ey)
    ### END YOUR CODE
...
def compute_cost(image, energy, axis=1):
    ...
    arrW = np.arange(W)
    for i in range(1,H):
        up = np.roll(cost[i-1],1)
        mid = cost[i-1]
        down = np.roll(cost[i-1],-1)
        up[0] = down[-1] = 500

        mixed = np.append(np.append([up], [mid],axis=0), [down],axis=0)
        argm = np.argmin(mixed,axis=0)

        cost[i] = energy[i] + mixed[argm, arrW]
        paths[i] = argm - 1
...
def remove_seam(image, seam):
    ...
    for i in range(H):
        for c in range(C):
            out[i,:,c] = np.delete(image[i,:,c], seam[i])
    ### END YOUR CODE
...
def reduce(image, size, axis=1, efunc=energy_function, cfunc=compute_cost, bfunc=backtrack_seam, rfunc=remove_seam):
    ...
    for i in range(W - size):
        en = efunc(out)
        cost, paths = cfunc(out, en)
        end = np.argmin(cost[-1])
        opt_seam = bfunc(paths, end)
        out = rfunc(out, opt_seam)
    ### END YOUR CODE
...
def duplicate_seam(image, seam):
    ...
    for i in range(H):
        for c in range(C):
            out[i,:,c] = np.insert(image[i,:,c], seam[i], image[i,seam[i],c])
    ### END YOUR CODE
...
def enlarge_naive(image, size, axis=1, efunc=energy_function, cfunc=compute_cost, bfunc=backtrack_seam, dfunc=duplicate_seam):
    ...
    for i in range(size - W):
        ...
        opt_seam = bfunc(paths, end)
        out = duplicate_seam(out, opt_seam)
    ### END YOUR CODE
...
def enlarge(image, size, axis=1, efunc=energy_function, cfunc=compute_cost, dfunc=duplicate_seam, bfunc=backtrack_seam, rfunc=remove_seam):
    ...
    curr_seam = np.zeros(H, dtype=np.int)

    for i in range(size - W):
        curr_seam = np.where(seams==i+1)[1]
        out = dfunc(out, curr_seam)

        seams = np.squeeze(dfunc(np.expand_dims(seams, axis=2), curr_seam))
...
def compute_forward_cost(image, energy):
    ...
    for i in range(1,H):
        for j in range(W):
            if j==0:
                left = 5000
                right = cost[i-1,j+1] + np.abs(image[i-1,j] - image[i,j+1])
                mid = cost[i-1,j]
            elif j==W-1:
                left = cost[i-1,j-1] + np.abs(image[i-1,j] - image[i,j-1])
                right = 5000
                mid = cost[i-1,j]
            else:
                left = cost[i-1,j-1] + np.abs(image[i,j+1] - image[i,j-1]) + np.abs(image[i-1,j] - image[i,j-1])
                right = cost[i-1,j+1] + np.abs(image[i,j+1] - image[i,j-1]) + np.abs(image[i-1,j] - image[i,j+1])
                mid = cost[i-1,j] + np.abs(image[i,j+1] - image[i,j-1])

            mixed = [left,mid,right]
            argm = np.argmin(mixed)

            cost[i,j] = energy[i,j] + mixed[argm]
            paths[i,j] = argm - 1
    ### END YOUR CODE
...
def remove_object(image, mask):
    ...
    counter = 0
    while np.any(mask):
        energy = energy_function(out) - 5000 * mask
        cost, paths = compute_cost(out, energy)
        end = np.argmin(cost[-1])
        opt_seam = backtrack_seam(paths, end)

        out = remove_seam(out, opt_seam)
        mask = remove_seam(mask, opt_seam)

        counter += 1

    out = enlarge(out, out.shape[1] + counter, cfunc=compute_cost)
    ### END YOUR CODE
