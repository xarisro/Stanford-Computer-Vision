...
class PCA(object):
    ...
    def _eigen_decomp(self, X):
        ...
        S = np.matmul(np.transpose(X),X) / N
    ...
    def _svd(self, X):
        ...
        M, P, NT = np.linalg.svd(X)

        vecs = np.transpose(NT)
        vals = P[0:min(N,D)]
        # END YOUR CODE
    ...
    def transform(self, X, n_components):
        ...
        X_centered = X - self.mean
        X_proj = np.matmul(X_centered, self.W_pca[:,0:n_components])
        # END YOUR CODE
    ...
    def reconstruct(self, X_proj):
        ...
        X_centered = np.matmul(X_proj, np.transpose(self.W_pca[:,0:n_components]))
...
class LDA(object):
    ...
    def fit(self, X, y):
        ...
        vals, vecs = scipy.linalg.eig(scatter_between, scatter_within)
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            S_i = np.matmul(np.transpose(X_i_centered),X_i_centered)
    ...
    def _between_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            scatter_between = scatter_between + N_i * np.matmul(mu_i - mu, np.transpose(mu_i - mu))
            # END YOUR CODE
    ...
    def transform(self, X, n_components):
        ...
        X_proj = np.matmul(X, self.W_lda[:,0:n_components])
        # END YOUR CODE
