...
def compute_distances(X1, X2):
    ...
    for i in range(M):
        dists[i,:] = np.linalg.norm(X1[i,:] - X2,axis=1)

    # dists = np.linalg.norm(np.repeat(X1,N,axis=0)-np.tile(X2,(M,1)),axis=1)
    # dists.resize((M,N))
    # END YOUR CODE
...
def predict_labels(dists, y_train, k=1):
    ...
    for i in range(num_test):
        ...
        freq = np.zeros(max(closest_y)+1,dtype=int)
        for el in closest_y:
            freq[el] = freq[el] + 1
...
def split_folds(X_train, y_train, num_folds):
    ...
    Xtsplit = np.array_split(X_train,num_folds)
    ytsplit = np.array_split(y_train,num_folds)

    for i in range(num_folds):
        X_trains[i,:,:] = np.concatenate([Xtsplit[j] for j in range(num_folds) if j != i])
        y_trains[i,:] = np.concatenate([ytsplit[j] for j in range(num_folds) if j != i])
