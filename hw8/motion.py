...
def lucas_kanade(img1, img2, keypoints, window_size=5):
    ...
    for y, x in keypoints:
        ...
        ### YOUR CODE HERE
        Iypi = [Iy[i,j] for i in range(y-w,y+w+1) for j in range(x-w,x+w+1)]
        Ixpi = [Ix[i,j] for i in range(y-w,y+w+1) for j in range(x-w,x+w+1)]
        Itpi = [It[i,j] for i in range(y-w,y+w+1) for j in range(x-w,x+w+1)]

        AT = np.array([Iypi, Ixpi])
        b = -np.array(Itpi)

        invv = np.linalg.inv(np.matmul(AT, np.transpose(AT)))
        u = np.matmul(invv, np.matmul(AT,b))
...
def iterative_lucas_kanade(img1, img2, keypoints,
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        ### YOUR CODE HERE
        Iypi = np.array([Iy[i,j] for i in range(y1-w,y1+w+1) for j in range(x1-w,x1+w+1)])
        Ixpi = np.array([Ix[i,j] for i in range(y1-w,y1+w+1) for j in range(x1-w,x1+w+1)])

        sumIy2 = np.sum(Iypi**2)
        sumIx2 = np.sum(Ixpi**2)
        sumIyIx = np.sum(Iypi * Ixpi)

        G = np.array([[sumIx2, sumIyIx], [sumIyIx, sumIy2]])
        ...
        for k in range(num_iters):
            ...
            ### YOUR CODE HERE
            im1patch = np.array([img1[i,j] for i in range(y1-w,y1+w+1) for j in range(x1-w,x1+w+1)])
            im2patch = np.array([img2[i,j] for i in range(y2-w,y2+w+1) for j in range(x2-w,x2+w+1)])
            
            dI = im1patch - im2patch

            bk = np.array([[np.sum(dI * Ixpi)],[np.sum(dI * Iypi)]])
            vk = np.matmul(invG, bk)
            vk = vk.reshape((2,))
            # print(vk.shape)
            ### END YOUR CODE
