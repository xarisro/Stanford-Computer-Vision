...
def harris_corners(img, window_size=3, k=0.04):
    ...
    convx = convolve(dx2, window, mode='constant')
    convy = convolve(dy2, window, mode='constant')
    convxy = convolve(dxy, window, mode='constant')

    response = convx * convy - convxy**2 - k * (convx + convy)**2
    ### END YOUR CODE
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    for i in range(dists.shape[0]):
    	pos1 = np.argmin(dists[i,:])
    	min1 = dists[i,pos1]
    	dists[i,pos1] = 1000000000000000000.0

    	pos2 = np.argmin(dists[i,:])
    	min2 = dists[i,pos2]

    	if (min1 / min2) < threshold:
    		matches.append((i, pos1))
    ### END YOUR CODE
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for k in range(n_iters):
    	idxs = np.random.randint(N, size=n_samples)
    	sample1 = matched1[idxs,:]
    	sample2 = matched2[idxs,:]

    	H = np.linalg.lstsq(sample2, sample1, rcond=None)[0]

    	n_inliers = 0
    	inliers = []
    	for i, (m1, m2) in enumerate(zip(matched1, matched2)):
    		if not (i in idxs):
    			m1id = np.dot(m2, H)
    			if np.linalg.norm(m1id-m1) < threshold:
    				n_inliers += 1
    				inliers.append(i)
    ...
    max_inliers = np.asarray(max_inliers)

    inl1 = matched1[max_inliers,:]
    inl2 = matched2[max_inliers,:]

    H = np.linalg.lstsq(inl2, inl1, rcond=None)[0]
    H[:,2] = np.array([0, 0, 1])
    ### END YOUR CODE
    print(H)
...
def hog_descriptor(patch, pixels_per_cell=(8,8)):
    ...
    for i in range(rows):
    	for j in range(cols):
    		G_cell = G_cells[i][j]
    		t_cell = theta_cells[i][j] // degrees_per_bin
    		t_cell[t_cell > 8] = 8
    		
    		# print(theta_cells[i][j])
    		for k in range(G_cell.shape[0]):
    			for l in range(G_cell.shape[1]):
    				cells[i][j][int(t_cell[k][l])] += G_cell[k][l]

    block = cells.flatten()
    b_mean = np.mean(block)
    b_std = np.std(block)

    block = (block - b_mean) / b_std
    ### YOUR CODE HERE
...
def linear_blend(img1_warped, img2_warped):
    ...
    img1_warped[img1_warped < 0] = 0
    img2_warped[img2_warped < 0] = 0
    ...
    weight1[0:left_margin] = 1
    weight1[left_margin:right_margin] = np.linspace(1.0, 0.0, num=(right_margin-left_margin))
    weight1 = np.tile(weight1,(out_H,1))
    
    weight2 = 1 - weight1

    w_img1 = img1_warped * weight1
    w_img2 = img2_warped * weight2

    merged = w_img1 + w_img2
    ### END YOUR CODE
    # print(left_margin, right_margin)
    # print(img1_warped[img1_warped >= 0])
    return merged
