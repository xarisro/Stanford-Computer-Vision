...
def sliding_window(image, base_score, stepSize, windowSize, pixel_per_cell=8):
    ...
    ### YOUR CODE HERE
    pH, pW = response_map.shape
    for i in range(pH):
    	for j in range(pW):
    		wind = pad_image[i*stepSize:i*stepSize+winH,j*stepSize:j*stepSize+winW]
    		win_hog, _ = hog_feature(wind,pixel_per_cell)
    		response_map[i,j] = np.dot(base_score, win_hog)

    mr, mc = np.unravel_index(np.argmax(response_map, axis=None), response_map.shape)
    max_score = response_map[mr,mc]

    maxr = mr * stepSize - winH // 2
    maxc = mc * stepSize - winW // 2
...
def pyramid(image, scale=0.9, minSize=(200, 100)):
    ...
    while True:
        ...
        ### YOUR CODE HERE
        ch, cw = image.shape
        if ch < minSize[0] or cw < minSize[1]:
        	break
        ### END YOUR CODE
...
def pyramid_score(image, base_score, shape, stepSize=20,
    ...
    ### YOUR CODE HERE
    for cur_scale, img in images:
    	ms, mr, mc, response_map = sliding_window(img, base_score, stepSize, shape, pixel_per_cell)
    	if ms > max_score:
    		max_score = ms
    		maxr = mr
    		maxc = mc
...
def compute_displacement(part_centers, face_shape):
    ...
    ### YOUR CODE HERE
    centr = np.array([face_shape[0]//2, face_shape[1]//2])
    d = centr - part_centers

    mu = np.mean(d,axis=0,dtype=np.int)
...
def gaussian_heatmap(heatmap_face, heatmaps, sigmas):
    ...
    for mapp, std in zip(heatmaps, sigmas):
    	gmap = gaussian(mapp,std)
    	heatmap += gmap

    r, c = np.unravel_index(np.argmax(heatmap, axis=None), heatmap.shape)
    ### END YOUR CODE
    return heatmap, r, c
